...
struct minus_core<P, F>::track_settings {
  track_settings():
    ...
    max_num_steps_(500),
    num_successes_before_increase_(5), // m2 numberSuccessesBeforeIncrease
...
template <problem P, typename F>
void minus_core<P, F>::evaluate_Hxt_constants(const C<F> * __restrict x /*x, t*/, const C<F> * __restrict params, C<F> * __restrict y)
{
  eval<P,F>::Hxt_constants(x, params, y);
}
...
struct minus_io_common {
  ...
  static void normalize_line(F l[ncoords2d_h]) { // we were making it unit
    ...
    const F nrm = std::sqrt(l[0]*l[0] + l[1]*l[1] + l[2]*l[2]);
